...
class SlapwarzPlugin(StandardCommand):
    ...
    def initialize(self, dbconn):
        ...
        with dbconn:
            dbconn.execute("create table if not exists SlapwarzWords(id integer primary key, type integer, word text, damage real)")
            dbconn.execute("create table if not exists SlapwarzSlaps(id integer primary key, ts datetime, who text, victim text, damage real)")
            dbconn.executemany("insert or ignore into SlapwarzWords values(?, ?, ?, ?)", [
                (0, 0, 'hardly', 14),
                (1, 0, 'accidently', 15),
                (2, 0, 'sadly', 25),
                (3, 0, 'funnily', 28),
    ...
    def execute_parsed(self, connection, event, extra, dbconn):
        ...
        damages = []
        with dbconn:
            c = dbconn.execute("select ts from SlapwarzSlaps where who=? and victim=? order by ts desc limit 1", (who, victim))
            res = c.fetchone()
            last_slap = res[0] if res is not None else 0

            for i in range(4):
                c = dbconn.execute("select word, damage from SlapwarzWords where type=? order by random() limit 1", (i,))
                res = c.fetchone()
                words.append(res[0])
                damages.append(res[1])
        ...
        if irc.client.is_channel(event.target) and victim in [u for u,m in connection.tehbot.users[event.target]]:
            if when - last_slap < self.timeout():
                ...
                with dbconn:
                    dbconn.execute("insert into SlapwarzSlaps values(null, ?, ?, ?, ?)", (when, who, victim, dmg))
        else:
            msg = slaptext + "."
